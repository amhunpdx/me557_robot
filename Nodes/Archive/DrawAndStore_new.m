%% Revised MATLAB Script for Freehand Drawing with 2D Drawing and Triple-Phase Lift-Off
% This version applies a rigid planar adjustment using the 4 corner offsets 
% (via bilinear interpolation) and adds a gravitational center offset 
% (bowl effect) using a single parameter (CenterZAdjust). The global BulkZAdjust
% is applied uniformly.
%
% The rigid planar effect means that each corner’s offset is applied in a linear fashion,
% with the effect decaying as you move away from that corner. The gravitational center
% offset gives extra influence to the center – maximum at (0,0) and decaying to 0 at the edges.
%
% Note on center parameters:
% - CenterZAdjust is now used to define the gravitational center offset.
%   (Previously, CenterWeightedZAdjust was used for a similar bowl effect.)
%   In this mixed approach, only one gravitational center parameter is needed.
%
% The final adjusted output (with x,y offsets and z skew corrections applied)
% is stored in the matrix "word".
